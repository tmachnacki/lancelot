...
export const publish = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (media.length === 0 || gig.description === "" || offers.length !== 3) {
      throw new ConvexError("Gig needs at least one image to be published");
    }
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (title.length > 60) {
      throw new ConvexError("Title cannot be longer than 60 characters");
    }
...
export const unfavorite = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!existingFavorite) {
      throw new ConvexError("Favorited gig not found");
    }
...
export const getCategoryAndSubcategory = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (!subcategory) {
      throw new ConvexError("Subcategory not found");
    }
